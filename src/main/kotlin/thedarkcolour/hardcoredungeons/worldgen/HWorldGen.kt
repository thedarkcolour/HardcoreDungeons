...
import thedarkcolour.hardcoredungeons.worldgen.configured
import thedarkcolour.hardcoredungeons.worldgen.decorated
import java.util.function.Supplier

@Suppress("HasPlatformType", "MemberVisibilityCanBePrivate")
object HWorldGen {
    val FEATURE_REGISTRY = FeatureRegistry(HardcoreDungeons.ID)

    // Overworld features
    val SHROOMY_BOULDER = FEATURE_REGISTRY.configuredFeature("shroomy_boulder") {
        Feature.FOREST_ROCK.configured(BlockStateConfiguration(HBlocks.SHROOMY_COBBLESTONE.block.defaultBlockState()))
    }
    val PLACED_SHROOMY_BOULDER = FEATURE_REGISTRY.placedFeature("placed_shroomy_boulder", Supplier<PlacedFeature> {
        val a = PlacedFeature((::SHROOMY_BOULDER.get().holder.get() as Holder<ConfiguredFeature<*, *>>), listOf(HeightmapPlacement.onHeightmap()))
        TODO()
        //PLACED_SHROOMY_BOULDER.decorated(Features.Placements.HEIGHTMAP_DOUBLE_SQUARE.countRandom(5))
    })
    ...
    fun <FC : FeatureConfiguration, F : Feature<FC>> registerConfiguredFeature(name: String, feature: ConfiguredFeature<FC, F>): ConfiguredFeature<FC, F> {
