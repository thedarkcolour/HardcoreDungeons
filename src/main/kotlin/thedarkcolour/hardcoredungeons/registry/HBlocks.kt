...
import thedarkcolour.hardcoredungeons.block.plant.misc.CornBlock
...
object HBlocks {
    ...
    val LUMLIGHT_DOOR = DoorBlock(Properties.create(Material.WOOD, MaterialColor.WHITE_TERRACOTTA).hardnessAndResistance(3.0F).sound(SoundType.WOOD).doesNotBlockMovement()).setRegistryKey("lumlight_door")
    val LUMLIGHT_TRAPDOOR = TrapDoorBlock(Properties.create(Material.WOOD, MaterialColor.WHITE_TERRACOTTA).hardnessAndResistance(3.0F).sound(SoundType.WOOD).doesNotBlockMovement()).setRegistryKey("lumlight_trapdoor")
    ...
    val RAINBOW_GLASS = GlassBlock(Properties.create(Material.GLASS, DyeColor.PURPLE).sound(SoundType.GLASS).hardnessAndResistance(0.3f, 100.0f).doesNotBlockMovement()).setRegistryKey("rainbow_glass")
    val RAINBOW_GLASS_PANE = PaneBlock(Properties.create(Material.GLASS, DyeColor.PURPLE).sound(SoundType.GLASS).hardnessAndResistance(0.3f, 100.0f).doesNotBlockMovement()).setRegistryKey("rainbow_glass_pane")
    ...
    fun registerBlocks(blocks: IForgeRegistry<Block>) {
        ...
        blocks.registerBlock(CASTLETON_BRICK_FENCE) // todo fence and wall models
        blocks.registerBlock(CASTLETON_BRICK_WALL) //
        ...
        blocks.registerBlock(CHARGED_CASTLETON_BRICK_WALL) //
        ...
        blocks.registerPillarBlock(
            STRIPPED_LUMLIGHT_LOG,
            end = "stripped_lumlight_log_top",
            side = "stripped_lumlight_log"
        )
        blocks.registerPillarBlock(
            STRIPPED_LUMLIGHT_WOOD,
            end = "stripped_lumlight_log",
            side = "stripped_lumlight_log"
        )
        ...
        blocks.registerBlock(LUMLIGHT_FENCE) // todo fix
        ...
        blocks.registerCubeColumn(LUMLIGHT_BOOKSHELF, LUMLIGHT_PLANKS)
        ...
        blocks.registerBlock(RAINBOW_BRICK_WALL) // todo
        blocks.registerBlock(RAINBOW_BRICK_FENCE) // todo
        ...
        blocks.registerBlock(RAINBOW_FACTORY_BRICK_WALL)
     ...
     */
    // todo shortcut function for logs and wood
    ...
    /**
     * Register a non-rotatable pillar block
     */
    // todo work like pillar except non-rotatable
    fun IForgeRegistry<Block>.registerCubeColumn(block: Block, end: Block) {
        ModelGenerator.CUBE_COLUMNS.add(block to end)
        this.registerBlock(block)
    }

    fun IForgeRegistry<Block>.registerDoorBlock(block: Block) {
        ModelGenerator.DOOR_BLOCKS.add(block)
        ModelGenerator.SIMPLE_ITEMS.add(block)
        this.registerBlock(block)
    }
     ...
     *
     * block (no block model) + item block
     */
