    ...
    override fun animateTick(state: BlockState, level: World, pos: BlockPos, rand: Random) {
        if (state.getValue(LIT)) {
            if (rand.nextInt(10) == 0) {
                level.playLocalSound((pos.x + 0.5f).toDouble(), (pos.y + 0.5f).toDouble(), (pos.z + 0.5f).toDouble(), SoundEvents.CAMPFIRE_CRACKLE, SoundCategory.BLOCKS, 0.5f + rand.nextFloat(), rand.nextFloat() * 0.7f + 0.6f, false)
            }
            if (rand.nextInt(5) == 0) {
