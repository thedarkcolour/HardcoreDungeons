...
class StairsSlabWallFenceCombo(name: String, properties: HProperties) : StairsSlabWallCombo(name, properties) {
    val fence by HBlockMaker.registerFence(HBlockMaker.getComboName(name) + "_fence", properties)

    init {
        ItemMaker.blockItem(HBlockMaker.getComboName(name) + "_fence", ItemModelType.WALL_FENCE_ITEM, ::fence)
    }
